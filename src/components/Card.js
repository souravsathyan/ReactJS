...
const Cards = ({ resData }) => {
    ...
    return (
        <div className='m-4 p-4  w-[280px] bg-slate-200 rounded-xl hover:shadow-xl'>
            ...
            <h4 className="bg-green-500 p-1 rounded-lg text-orange-600 font-semibold" ><i class="font-thin fa-solid fa-star"></i>{avgRating}</h4>
            </div>
            <div className="px-1">
            <p className="break-words">{cuisines.slice(0,3).join(',')}</p>
            <p>{sla.slaString} <i class="fa-regular fa-clock"></i></p>
            </div>
        </div>
    )
}
